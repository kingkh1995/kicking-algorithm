...
public class SortingExs {
  ...
  /**
   * 1、一直查看最上面的两张牌，将小牌放到最下面，直到找到最大的牌和第二大的牌 2、将第二大的牌和最大的牌放在最下面，使他们有序的连在一起 3、继续通过以上方法找出第三大的牌和第二大的牌
   * 4、可以知道最大的牌就跟在第二大的牌后面，同样将它们都放到最下面，使前三张牌有序的连在一起 5、一直重复，并按顺序使所有的牌全部排序并连在一起，相当于组成了一个环
   */
  /** 拓展：队列排序（无重复数据） 类似于选择排序 */
  ...
  /**
   * 1、将数组按M大小分块，每块使用选择排序或插入排序 2、归并第一个子数组和第二个子数组 3、循环归并前面所有归并完的数组和下一个未归并的子数组 归并方法是：
   * 将右边的M大小的子数组复制到M大小的辅助数组中，然后从两个数组右边开始取值，将大值放到右边数组原区间的最右边，
   * 直到右边数组全部归并完成，因为左右数组都是排序的，所以左边数组剩余的部分也是有序的且全部小于右边数组 可知该算法的辅助数组大小永远等于M
   */

  // ==============================================================================================

  /** 2.2.16 自然的归并排序 编写一个自底向上的归并排序，能够利用数组中有序的部分，连续归并两个递增的区块 */
  ...
  /**
   * 2.3.24 取样排序 取样大小为2^k-1 先将取样的元素排序，再以中位数分割分为两个子数组移动到数组的左右两端，则数组分为了四个部分：有序的取样数组、
   * 取样数组的中位数、无序的剩余数组、有序的取样数组。使用中位数作pivot，对未取样的剩余元素做快速排序进行切分
   * 切分完成后，接下来我们再对第一个部分取半，放到中位数之前；对最后一部分取半，放到中位数之后 则此时中位数左右的数组都被分为了上一步一样的四部分，重复以上过程，直至完全排序
   */

  // ==============================================================================================

  /** 2.4.26 无需交换的堆 类似于插入排序的优化解法，使用半交换方式，每次只赋值被下沉或上浮的值，然后保存下待上浮或下沉的值 */
  ...
  /**
   * 首先将所有任务按运行时间排序，为处理器构建一个M大小的最小堆，记录下处理器接受到任务所需的总时间 将排序后的任务数组，按顺序开始分配，每次从堆顶取出当前总时间最小的处理器，分配当前任务，
  ...
  /** 2.5.23 选择的取样 找出第K小的元素，使用取样改进算法 Floyd-Rivest算法 */
