...
public class Essences {
    ...
    /**
     * 1.4.24
     * 扔鸡蛋问题 N层高的楼 鸡蛋从F楼扔下会摔碎 求F
     * 碎掉的鸡蛋为对数级别：二分查找
     * 碎掉的鸡蛋为 lgF级别：从第一层开始每次上移2的幂次方，摔碎了则可以确定F的区间，该区间最坏的情况下长度等于F，然后使用二分查找找到F
     * 故摔碎的次数为 lgF级别
     */
    /**
     * 1.4.25
     * 只有两个鸡蛋 如何求出F 表示最多只能摔碎两次。
     * 解法一：可以将这个问题转化 ，2个鸡蛋测试k次最多能测试多少层
     * 	因为只能测试 k 次，因此我们第一个鸡蛋选择楼层的时候要慎重， 设第一次选择的m层
     * 	如果第一次扔鸡蛋碎了，那么只能从最底层开始轮询，则最坏的情况下需要测试 m -1次才能测出来，所以为了确保能在k次完成，第一次的选择必然是k层
     * 	如果鸡蛋没碎，此时还剩下k-1次测试机会，所以选择 k+(k-1)层
     * 	因此最后的方程应该是，楼层总数h = k+(k-1)+(k-2)+···+1;
     * 	所以只需要能判断的楼层高度h大于N,就能一定能在k次测出，(1+k)*k/2>N，所以可以得出测出N层楼的最大次数
     * 	测100层最多需要14次 14+13+12+...+1
     *
     *  解法二：看做判断2个鸡蛋要测到N层最多需要多少次？以N为标准，从最底层开始测。
